(() => {
  ...
  function extractPostsFromHtml() {
    ...
    const posts = [];
    const seen = new Set();
    const pattern = /(?:https?:\/\/www\.xiaohongshu\.com)?\/(?:explore|discovery\/item|search_result)\/([0-9a-fA-F]{12,32})(?:\?[^"'<>\\\s]*)?/g;
    for (const html of htmlVariants) {
      for (const match of html.matchAll(pattern)) {
        const postId = match[1];
        const url = normalizeExtractedPostUrl(match[0], postId);
        const key = `${postId}:${url.includes("xsec_token") ? "token" : "plain"}`;
        if (!postId || seen.has(key)) {
          continue;
        }
        seen.add(key);
        posts.push({
